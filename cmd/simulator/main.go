...
var printStateSampleSize = flag.Int("print-state-every", 512, "how often in steps to print the computer state")

func main() {
	flag.Parse()
	fmt.Println("\nSimple Computer (based on the Scott CPU)")
...
func run(bin []uint16) {
	keyPressChannel := make(chan *sio.KeyPress)
	...
	if err := glfw.Init(fmt.Sprintf("%s", *binFile)); err != nil {
		fmt.Fprintln(os.Stderr, "error received initialising GLFW instance", err)
		os.Exit(5)
	}

	comp := computer.NewComputer(screenChannel, quitChannel)
	keyboard := sio.NewKeyboard(keyPressChannel, quitChannel)
