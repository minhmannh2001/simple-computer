%LINE-WIDTH = 0x1E
%CALL-RETURN-ADDRESS = 0xFF33
%ONE = 0x1
%LINEX = 0xFF01
%PEN-POSITION-ADDR = 0x400
%KEYCODE-REGISTER = 0x401
%DISPLAY-ADAPTER-ADDR = 0x7
%KEY-ADAPTER-ADDR = 0xF
DATA R0, %LINEX
DATA R1, 0x0
ST R0, R1
start:
CALL ROUTINE-init-fontDescriptions
JMP main
ROUTINES:
ROUTINE-init-fontDescriptions:
DATA R0, 0x3E8
DATA R1, 0x30
ST R0, R1
DATA R0, 0x3E9
DATA R1, 0x8
ST R0, R1
DATA R0, 0x3EA
DATA R1, 0xC
ST R0, R1
DATA R0, 0x3EB
DATA R1, 0x2
ST R0, R1
DATA R0, 0x3EC
DATA R1, 0xC
ST R0, R1
DATA R0, 0x3ED
DATA R1, 0x8
ST R0, R1
DATA R0, 0x3EE
DATA R1, 0x30
ST R0, R1
DATA R0, 0x3EF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2A8
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x2A9
DATA R1, 0x42
ST R0, R1
DATA R0, 0x2AA
DATA R1, 0x42
ST R0, R1
DATA R0, 0x2AB
DATA R1, 0x42
ST R0, R1
DATA R0, 0x2AC
DATA R1, 0x42
ST R0, R1
DATA R0, 0x2AD
DATA R1, 0x42
ST R0, R1
DATA R0, 0x2AE
DATA R1, 0x3C
ST R0, R1
DATA R0, 0x2AF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x370
DATA R1, 0xC2
ST R0, R1
DATA R0, 0x371
DATA R1, 0xA2
ST R0, R1
DATA R0, 0x372
DATA R1, 0x92
ST R0, R1
DATA R0, 0x373
DATA R1, 0x92
ST R0, R1
DATA R0, 0x374
DATA R1, 0x8A
ST R0, R1
DATA R0, 0x375
DATA R1, 0x8A
ST R0, R1
DATA R0, 0x376
DATA R1, 0x86
ST R0, R1
DATA R0, 0x377
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1C0
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x1C1
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1C2
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1C3
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x1C4
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1C5
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1C6
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x1C7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x110
DATA R1, 0x28
ST R0, R1
DATA R0, 0x111
DATA R1, 0x28
ST R0, R1
DATA R0, 0x112
DATA R1, 0x0
ST R0, R1
DATA R0, 0x113
DATA R1, 0x0
ST R0, R1
DATA R0, 0x114
DATA R1, 0x0
ST R0, R1
DATA R0, 0x115
DATA R1, 0x0
ST R0, R1
DATA R0, 0x116
DATA R1, 0x0
ST R0, R1
DATA R0, 0x117
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3E0
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E1
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E2
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E4
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E5
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E6
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3E7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3F0
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3F1
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3F2
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3F3
DATA R1, 0x32
ST R0, R1
DATA R0, 0x3F4
DATA R1, 0x4C
ST R0, R1
DATA R0, 0x3F5
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3F6
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3F7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F8
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F9
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2FA
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2FB
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2FC
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2FD
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2FE
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x2FF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x278
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x279
DATA R1, 0x82
ST R0, R1
DATA R0, 0x27A
DATA R1, 0x82
ST R0, R1
DATA R0, 0x27B
DATA R1, 0x82
ST R0, R1
DATA R0, 0x27C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x27D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x27E
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x27F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2C0
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x2C1
DATA R1, 0x44
ST R0, R1
DATA R0, 0x2C2
DATA R1, 0x28
ST R0, R1
DATA R0, 0x2C3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2C4
DATA R1, 0x28
ST R0, R1
DATA R0, 0x2C5
DATA R1, 0x44
ST R0, R1
DATA R0, 0x2C6
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x2C7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1A8
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x1A9
DATA R1, 0x80
ST R0, R1
DATA R0, 0x1AA
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x1AB
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1AC
DATA R1, 0x2
ST R0, R1
DATA R0, 0x1AD
DATA R1, 0x6
ST R0, R1
DATA R0, 0x1AE
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x1AF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1C8
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x1C9
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1CA
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1CB
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x1CC
DATA R1, 0x2
ST R0, R1
DATA R0, 0x1CD
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1CE
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x1CF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x138
DATA R1, 0x20
ST R0, R1
DATA R0, 0x139
DATA R1, 0x20
ST R0, R1
DATA R0, 0x13A
DATA R1, 0x20
ST R0, R1
DATA R0, 0x13B
DATA R1, 0x0
ST R0, R1
DATA R0, 0x13C
DATA R1, 0x0
ST R0, R1
DATA R0, 0x13D
DATA R1, 0x0
ST R0, R1
DATA R0, 0x13E
DATA R1, 0x0
ST R0, R1
DATA R0, 0x13F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x148
DATA R1, 0x20
ST R0, R1
DATA R0, 0x149
DATA R1, 0x10
ST R0, R1
DATA R0, 0x14A
DATA R1, 0x8
ST R0, R1
DATA R0, 0x14B
DATA R1, 0x8
ST R0, R1
DATA R0, 0x14C
DATA R1, 0x8
ST R0, R1
DATA R0, 0x14D
DATA R1, 0x10
ST R0, R1
DATA R0, 0x14E
DATA R1, 0x20
ST R0, R1
DATA R0, 0x14F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x290
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x291
DATA R1, 0x82
ST R0, R1
DATA R0, 0x292
DATA R1, 0x82
ST R0, R1
DATA R0, 0x293
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x294
DATA R1, 0xA0
ST R0, R1
DATA R0, 0x295
DATA R1, 0x90
ST R0, R1
DATA R0, 0x296
DATA R1, 0x8E
ST R0, R1
DATA R0, 0x297
DATA R1, 0x0
ST R0, R1
DATA R0, 0x180
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x181
DATA R1, 0xE2
ST R0, R1
DATA R0, 0x182
DATA R1, 0xA2
ST R0, R1
DATA R0, 0x183
DATA R1, 0x92
ST R0, R1
DATA R0, 0x184
DATA R1, 0x8A
ST R0, R1
DATA R0, 0x185
DATA R1, 0x8E
ST R0, R1
DATA R0, 0x186
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x187
DATA R1, 0x0
ST R0, R1
DATA R0, 0x0
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x1
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x2
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x3
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x4
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x5
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x6
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x7
DATA R1, 0xFFFF
ST R0, R1
DATA R0, 0x120
DATA R1, 0x10
ST R0, R1
DATA R0, 0x121
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x122
DATA R1, 0x90
ST R0, R1
DATA R0, 0x123
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x124
DATA R1, 0x12
ST R0, R1
DATA R0, 0x125
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x126
DATA R1, 0x10
ST R0, R1
DATA R0, 0x127
DATA R1, 0x0
ST R0, R1
DATA R0, 0x250
DATA R1, 0x2
ST R0, R1
DATA R0, 0x251
DATA R1, 0x2
ST R0, R1
DATA R0, 0x252
DATA R1, 0x2
ST R0, R1
DATA R0, 0x253
DATA R1, 0x2
ST R0, R1
DATA R0, 0x254
DATA R1, 0x2
ST R0, R1
DATA R0, 0x255
DATA R1, 0x2
ST R0, R1
DATA R0, 0x256
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x257
DATA R1, 0x0
ST R0, R1
DATA R0, 0x280
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x281
DATA R1, 0x82
ST R0, R1
DATA R0, 0x282
DATA R1, 0x82
ST R0, R1
DATA R0, 0x283
DATA R1, 0x1FC
ST R0, R1
DATA R0, 0x284
DATA R1, 0x80
ST R0, R1
DATA R0, 0x285
DATA R1, 0x80
ST R0, R1
DATA R0, 0x286
DATA R1, 0x80
ST R0, R1
DATA R0, 0x287
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2B0
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2B1
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2B2
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2B3
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2B4
DATA R1, 0x44
ST R0, R1
DATA R0, 0x2B5
DATA R1, 0x6C
ST R0, R1
DATA R0, 0x2B6
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2B7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x308
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x309
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x30A
DATA R1, 0x82
ST R0, R1
DATA R0, 0x30B
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x30C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x30D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x30E
DATA R1, 0x82
ST R0, R1
DATA R0, 0x30F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x358
DATA R1, 0xC4
ST R0, R1
DATA R0, 0x359
DATA R1, 0xC8
ST R0, R1
DATA R0, 0x35A
DATA R1, 0xF0
ST R0, R1
DATA R0, 0x35B
DATA R1, 0xE0
ST R0, R1
DATA R0, 0x35C
DATA R1, 0xD8
ST R0, R1
DATA R0, 0x35D
DATA R1, 0xC4
ST R0, R1
DATA R0, 0x35E
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x35F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3A8
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x3A9
DATA R1, 0x42
ST R0, R1
DATA R0, 0x3AA
DATA R1, 0x42
ST R0, R1
DATA R0, 0x3AB
DATA R1, 0x42
ST R0, R1
DATA R0, 0x3AC
DATA R1, 0x42
ST R0, R1
DATA R0, 0x3AD
DATA R1, 0x42
ST R0, R1
DATA R0, 0x3AE
DATA R1, 0x3C
ST R0, R1
DATA R0, 0x3AF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2A0
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x2A1
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2A2
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2A3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2A4
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2A5
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2A6
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2A7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x330
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x331
DATA R1, 0x80
ST R0, R1
DATA R0, 0x332
DATA R1, 0x80
ST R0, R1
DATA R0, 0x333
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x334
DATA R1, 0x80
ST R0, R1
DATA R0, 0x335
DATA R1, 0x80
ST R0, R1
DATA R0, 0x336
DATA R1, 0x80
ST R0, R1
DATA R0, 0x337
DATA R1, 0x0
ST R0, R1
DATA R0, 0x340
DATA R1, 0x82
ST R0, R1
DATA R0, 0x341
DATA R1, 0x82
ST R0, R1
DATA R0, 0x342
DATA R1, 0x82
ST R0, R1
DATA R0, 0x343
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x344
DATA R1, 0x82
ST R0, R1
DATA R0, 0x345
DATA R1, 0x82
ST R0, R1
DATA R0, 0x346
DATA R1, 0x82
ST R0, R1
DATA R0, 0x347
DATA R1, 0x0
ST R0, R1
DATA R0, 0x350
DATA R1, 0x2
ST R0, R1
DATA R0, 0x351
DATA R1, 0x2
ST R0, R1
DATA R0, 0x352
DATA R1, 0x2
ST R0, R1
DATA R0, 0x353
DATA R1, 0x2
ST R0, R1
DATA R0, 0x354
DATA R1, 0x2
ST R0, R1
DATA R0, 0x355
DATA R1, 0x2
ST R0, R1
DATA R0, 0x356
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x357
DATA R1, 0x0
ST R0, R1
DATA R0, 0x380
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x381
DATA R1, 0x82
ST R0, R1
DATA R0, 0x382
DATA R1, 0x82
ST R0, R1
DATA R0, 0x383
DATA R1, 0x1FC
ST R0, R1
DATA R0, 0x384
DATA R1, 0x80
ST R0, R1
DATA R0, 0x385
DATA R1, 0x80
ST R0, R1
DATA R0, 0x386
DATA R1, 0x80
ST R0, R1
DATA R0, 0x387
DATA R1, 0x0
ST R0, R1
DATA R0, 0x150
DATA R1, 0x0
ST R0, R1
DATA R0, 0x151
DATA R1, 0x92
ST R0, R1
DATA R0, 0x152
DATA R1, 0x54
ST R0, R1
DATA R0, 0x153
DATA R1, 0x38
ST R0, R1
DATA R0, 0x154
DATA R1, 0x38
ST R0, R1
DATA R0, 0x155
DATA R1, 0x54
ST R0, R1
DATA R0, 0x156
DATA R1, 0x92
ST R0, R1
DATA R0, 0x157
DATA R1, 0x0
ST R0, R1
DATA R0, 0x178
DATA R1, 0x2
ST R0, R1
DATA R0, 0x179
DATA R1, 0x4
ST R0, R1
DATA R0, 0x17A
DATA R1, 0x8
ST R0, R1
DATA R0, 0x17B
DATA R1, 0x10
ST R0, R1
DATA R0, 0x17C
DATA R1, 0x20
ST R0, R1
DATA R0, 0x17D
DATA R1, 0x40
ST R0, R1
DATA R0, 0x17E
DATA R1, 0x80
ST R0, R1
DATA R0, 0x17F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1E8
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1E9
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1EA
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x1EB
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1EC
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x1ED
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1EE
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1EF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1E0
DATA R1, 0x2
ST R0, R1
DATA R0, 0x1E1
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1E2
DATA R1, 0x8
ST R0, R1
DATA R0, 0x1E3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1E4
DATA R1, 0x8
ST R0, R1
DATA R0, 0x1E5
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1E6
DATA R1, 0x2
ST R0, R1
DATA R0, 0x1E7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x300
DATA R1, 0x0
ST R0, R1
DATA R0, 0x301
DATA R1, 0x20
ST R0, R1
DATA R0, 0x302
DATA R1, 0x10
ST R0, R1
DATA R0, 0x303
DATA R1, 0x8
ST R0, R1
DATA R0, 0x304
DATA R1, 0x0
ST R0, R1
DATA R0, 0x305
DATA R1, 0x0
ST R0, R1
DATA R0, 0x306
DATA R1, 0x0
ST R0, R1
DATA R0, 0x307
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D0
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D1
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1D2
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D3
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D4
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1D5
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D6
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1A0
DATA R1, 0x1C
ST R0, R1
DATA R0, 0x1A1
DATA R1, 0x24
ST R0, R1
DATA R0, 0x1A2
DATA R1, 0x44
ST R0, R1
DATA R0, 0x1A3
DATA R1, 0x84
ST R0, R1
DATA R0, 0x1A4
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x1A5
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1A6
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1A7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x338
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x339
DATA R1, 0x80
ST R0, R1
DATA R0, 0x33A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x33B
DATA R1, 0x9C
ST R0, R1
DATA R0, 0x33C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x33D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x33E
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x33F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x360
DATA R1, 0x80
ST R0, R1
DATA R0, 0x361
DATA R1, 0x80
ST R0, R1
DATA R0, 0x362
DATA R1, 0x80
ST R0, R1
DATA R0, 0x363
DATA R1, 0x80
ST R0, R1
DATA R0, 0x364
DATA R1, 0x80
ST R0, R1
DATA R0, 0x365
DATA R1, 0x80
ST R0, R1
DATA R0, 0x366
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x367
DATA R1, 0x0
ST R0, R1
DATA R0, 0x158
DATA R1, 0x0
ST R0, R1
DATA R0, 0x159
DATA R1, 0x10
ST R0, R1
DATA R0, 0x15A
DATA R1, 0x10
ST R0, R1
DATA R0, 0x15B
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x15C
DATA R1, 0x30
ST R0, R1
DATA R0, 0x15D
DATA R1, 0x10
ST R0, R1
DATA R0, 0x15E
DATA R1, 0x0
ST R0, R1
DATA R0, 0x15F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F0
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2F1
DATA R1, 0x28
ST R0, R1
DATA R0, 0x2F2
DATA R1, 0x44
ST R0, R1
DATA R0, 0x2F3
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F4
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F5
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F6
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2F7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x238
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x239
DATA R1, 0x80
ST R0, R1
DATA R0, 0x23A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x23B
DATA R1, 0x9C
ST R0, R1
DATA R0, 0x23C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x23D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x23E
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x23F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x260
DATA R1, 0x80
ST R0, R1
DATA R0, 0x261
DATA R1, 0x80
ST R0, R1
DATA R0, 0x262
DATA R1, 0x80
ST R0, R1
DATA R0, 0x263
DATA R1, 0x80
ST R0, R1
DATA R0, 0x264
DATA R1, 0x80
ST R0, R1
DATA R0, 0x265
DATA R1, 0x80
ST R0, R1
DATA R0, 0x266
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x267
DATA R1, 0x0
ST R0, R1
DATA R0, 0x378
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x379
DATA R1, 0x82
ST R0, R1
DATA R0, 0x37A
DATA R1, 0x82
ST R0, R1
DATA R0, 0x37B
DATA R1, 0x82
ST R0, R1
DATA R0, 0x37C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x37D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x37E
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x37F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3B8
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3B9
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3BA
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3BB
DATA R1, 0x92
ST R0, R1
DATA R0, 0x3BC
DATA R1, 0xBA
ST R0, R1
DATA R0, 0x3BD
DATA R1, 0xAA
ST R0, R1
DATA R0, 0x3BE
DATA R1, 0x44
ST R0, R1
DATA R0, 0x3BF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3D0
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x3D1
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3D2
DATA R1, 0xC
ST R0, R1
DATA R0, 0x3D3
DATA R1, 0x38
ST R0, R1
DATA R0, 0x3D4
DATA R1, 0x60
ST R0, R1
DATA R0, 0x3D5
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3D6
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x3D7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x118
DATA R1, 0x28
ST R0, R1
DATA R0, 0x119
DATA R1, 0x28
ST R0, R1
DATA R0, 0x11A
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x11B
DATA R1, 0x28
ST R0, R1
DATA R0, 0x11C
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x11D
DATA R1, 0x28
ST R0, R1
DATA R0, 0x11E
DATA R1, 0x28
ST R0, R1
DATA R0, 0x11F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x168
DATA R1, 0x0
ST R0, R1
DATA R0, 0x169
DATA R1, 0x0
ST R0, R1
DATA R0, 0x16A
DATA R1, 0x0
ST R0, R1
DATA R0, 0x16B
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x16C
DATA R1, 0x0
ST R0, R1
DATA R0, 0x16D
DATA R1, 0x0
ST R0, R1
DATA R0, 0x16E
DATA R1, 0x0
ST R0, R1
DATA R0, 0x16F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2D8
DATA R1, 0x30
ST R0, R1
DATA R0, 0x2D9
DATA R1, 0x20
ST R0, R1
DATA R0, 0x2DA
DATA R1, 0x20
ST R0, R1
DATA R0, 0x2DB
DATA R1, 0x20
ST R0, R1
DATA R0, 0x2DC
DATA R1, 0x20
ST R0, R1
DATA R0, 0x2DD
DATA R1, 0x20
ST R0, R1
DATA R0, 0x2DE
DATA R1, 0x30
ST R0, R1
DATA R0, 0x2DF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2C8
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x2C9
DATA R1, 0x44
ST R0, R1
DATA R0, 0x2CA
DATA R1, 0x28
ST R0, R1
DATA R0, 0x2CB
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2CC
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2CD
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2CE
DATA R1, 0x38
ST R0, R1
DATA R0, 0x2CF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x318
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x319
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x31A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x31B
DATA R1, 0x80
ST R0, R1
DATA R0, 0x31C
DATA R1, 0x80
ST R0, R1
DATA R0, 0x31D
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x31E
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x31F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3A0
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x3A1
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3A2
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3A3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3A4
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3A5
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3A6
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3A7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x198
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x199
DATA R1, 0x2
ST R0, R1
DATA R0, 0x19A
DATA R1, 0x2
ST R0, R1
DATA R0, 0x19B
DATA R1, 0x1E
ST R0, R1
DATA R0, 0x19C
DATA R1, 0x2
ST R0, R1
DATA R0, 0x19D
DATA R1, 0x2
ST R0, R1
DATA R0, 0x19E
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x19F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x100
DATA R1, 0x0
ST R0, R1
DATA R0, 0x101
DATA R1, 0x0
ST R0, R1
DATA R0, 0x102
DATA R1, 0x0
ST R0, R1
DATA R0, 0x103
DATA R1, 0x0
ST R0, R1
DATA R0, 0x104
DATA R1, 0x0
ST R0, R1
DATA R0, 0x105
DATA R1, 0x0
ST R0, R1
DATA R0, 0x106
DATA R1, 0x0
ST R0, R1
DATA R0, 0x107
DATA R1, 0x0
ST R0, R1
DATA R0, 0x130
DATA R1, 0x38
ST R0, R1
DATA R0, 0x131
DATA R1, 0x28
ST R0, R1
DATA R0, 0x132
DATA R1, 0x38
ST R0, R1
DATA R0, 0x133
DATA R1, 0xE0
ST R0, R1
DATA R0, 0x134
DATA R1, 0x94
ST R0, R1
DATA R0, 0x135
DATA R1, 0x88
ST R0, R1
DATA R0, 0x136
DATA R1, 0xF4
ST R0, R1
DATA R0, 0x137
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2E0
DATA R1, 0x80
ST R0, R1
DATA R0, 0x2E1
DATA R1, 0x40
ST R0, R1
DATA R0, 0x2E2
DATA R1, 0x20
ST R0, R1
DATA R0, 0x2E3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2E4
DATA R1, 0x8
ST R0, R1
DATA R0, 0x2E5
DATA R1, 0x4
ST R0, R1
DATA R0, 0x2E6
DATA R1, 0x2
ST R0, R1
DATA R0, 0x2E7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1F8
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x1F9
DATA R1, 0x42
ST R0, R1
DATA R0, 0x1FA
DATA R1, 0x2
ST R0, R1
DATA R0, 0x1FB
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1FC
DATA R1, 0x8
ST R0, R1
DATA R0, 0x1FD
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1FE
DATA R1, 0x8
ST R0, R1
DATA R0, 0x1FF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x220
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x221
DATA R1, 0x86
ST R0, R1
DATA R0, 0x222
DATA R1, 0x82
ST R0, R1
DATA R0, 0x223
DATA R1, 0x82
ST R0, R1
DATA R0, 0x224
DATA R1, 0x82
ST R0, R1
DATA R0, 0x225
DATA R1, 0x86
ST R0, R1
DATA R0, 0x226
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x227
DATA R1, 0x0
ST R0, R1
DATA R0, 0x230
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x231
DATA R1, 0x80
ST R0, R1
DATA R0, 0x232
DATA R1, 0x80
ST R0, R1
DATA R0, 0x233
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x234
DATA R1, 0x80
ST R0, R1
DATA R0, 0x235
DATA R1, 0x80
ST R0, R1
DATA R0, 0x236
DATA R1, 0x80
ST R0, R1
DATA R0, 0x237
DATA R1, 0x0
ST R0, R1
DATA R0, 0x270
DATA R1, 0xC2
ST R0, R1
DATA R0, 0x271
DATA R1, 0xA2
ST R0, R1
DATA R0, 0x272
DATA R1, 0x92
ST R0, R1
DATA R0, 0x273
DATA R1, 0x92
ST R0, R1
DATA R0, 0x274
DATA R1, 0x8A
ST R0, R1
DATA R0, 0x275
DATA R1, 0x8A
ST R0, R1
DATA R0, 0x276
DATA R1, 0x86
ST R0, R1
DATA R0, 0x277
DATA R1, 0x0
ST R0, R1
DATA R0, 0x320
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x321
DATA R1, 0x86
ST R0, R1
DATA R0, 0x322
DATA R1, 0x82
ST R0, R1
DATA R0, 0x323
DATA R1, 0x82
ST R0, R1
DATA R0, 0x324
DATA R1, 0x82
ST R0, R1
DATA R0, 0x325
DATA R1, 0x86
ST R0, R1
DATA R0, 0x326
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x327
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D8
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1D9
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1DA
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1DB
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1DC
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1DD
DATA R1, 0x20
ST R0, R1
DATA R0, 0x1DE
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1DF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x200
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x201
DATA R1, 0x8A
ST R0, R1
DATA R0, 0x202
DATA R1, 0x9C
ST R0, R1
DATA R0, 0x203
DATA R1, 0xA8
ST R0, R1
DATA R0, 0x204
DATA R1, 0x98
ST R0, R1
DATA R0, 0x205
DATA R1, 0x84
ST R0, R1
DATA R0, 0x206
DATA R1, 0x78
ST R0, R1
DATA R0, 0x207
DATA R1, 0x0
ST R0, R1
DATA R0, 0x248
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x249
DATA R1, 0x10
ST R0, R1
DATA R0, 0x24A
DATA R1, 0x10
ST R0, R1
DATA R0, 0x24B
DATA R1, 0x10
ST R0, R1
DATA R0, 0x24C
DATA R1, 0x10
ST R0, R1
DATA R0, 0x24D
DATA R1, 0x10
ST R0, R1
DATA R0, 0x24E
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x24F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x268
DATA R1, 0x66
ST R0, R1
DATA R0, 0x269
DATA R1, 0xAA
ST R0, R1
DATA R0, 0x26A
DATA R1, 0x92
ST R0, R1
DATA R0, 0x26B
DATA R1, 0x92
ST R0, R1
DATA R0, 0x26C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x26D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x26E
DATA R1, 0x82
ST R0, R1
DATA R0, 0x26F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x298
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x299
DATA R1, 0x80
ST R0, R1
DATA R0, 0x29A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x29B
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x29C
DATA R1, 0x4
ST R0, R1
DATA R0, 0x29D
DATA R1, 0x4
ST R0, R1
DATA R0, 0x29E
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x29F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x310
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x311
DATA R1, 0x86
ST R0, R1
DATA R0, 0x312
DATA R1, 0x82
ST R0, R1
DATA R0, 0x313
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x314
DATA R1, 0x82
ST R0, R1
DATA R0, 0x315
DATA R1, 0x86
ST R0, R1
DATA R0, 0x316
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x317
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3B0
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3B1
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3B2
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3B3
DATA R1, 0x82
ST R0, R1
DATA R0, 0x3B4
DATA R1, 0x44
ST R0, R1
DATA R0, 0x3B5
DATA R1, 0x6C
ST R0, R1
DATA R0, 0x3B6
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3B7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1B8
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x1B9
DATA R1, 0x2
ST R0, R1
DATA R0, 0x1BA
DATA R1, 0x4
ST R0, R1
DATA R0, 0x1BB
DATA R1, 0x8
ST R0, R1
DATA R0, 0x1BC
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1BD
DATA R1, 0x20
ST R0, R1
DATA R0, 0x1BE
DATA R1, 0x40
ST R0, R1
DATA R0, 0x1BF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x170
DATA R1, 0x0
ST R0, R1
DATA R0, 0x171
DATA R1, 0x0
ST R0, R1
DATA R0, 0x172
DATA R1, 0x0
ST R0, R1
DATA R0, 0x173
DATA R1, 0x0
ST R0, R1
DATA R0, 0x174
DATA R1, 0x0
ST R0, R1
DATA R0, 0x175
DATA R1, 0x0
ST R0, R1
DATA R0, 0x176
DATA R1, 0x10
ST R0, R1
DATA R0, 0x177
DATA R1, 0x0
ST R0, R1
DATA R0, 0x160
DATA R1, 0x0
ST R0, R1
DATA R0, 0x161
DATA R1, 0x0
ST R0, R1
DATA R0, 0x162
DATA R1, 0x0
ST R0, R1
DATA R0, 0x163
DATA R1, 0x0
ST R0, R1
DATA R0, 0x164
DATA R1, 0x8
ST R0, R1
DATA R0, 0x165
DATA R1, 0x8
ST R0, R1
DATA R0, 0x166
DATA R1, 0x10
ST R0, R1
DATA R0, 0x167
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1F0
DATA R1, 0x40
ST R0, R1
DATA R0, 0x1F1
DATA R1, 0x20
ST R0, R1
DATA R0, 0x1F2
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1F3
DATA R1, 0x8
ST R0, R1
DATA R0, 0x1F4
DATA R1, 0x10
ST R0, R1
DATA R0, 0x1F5
DATA R1, 0x20
ST R0, R1
DATA R0, 0x1F6
DATA R1, 0x40
ST R0, R1
DATA R0, 0x1F7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2E8
DATA R1, 0x30
ST R0, R1
DATA R0, 0x2E9
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2EA
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2EB
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2EC
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2ED
DATA R1, 0x10
ST R0, R1
DATA R0, 0x2EE
DATA R1, 0x30
ST R0, R1
DATA R0, 0x2EF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3D8
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3D9
DATA R1, 0x20
ST R0, R1
DATA R0, 0x3DA
DATA R1, 0x60
ST R0, R1
DATA R0, 0x3DB
DATA R1, 0x80
ST R0, R1
DATA R0, 0x3DC
DATA R1, 0x60
ST R0, R1
DATA R0, 0x3DD
DATA R1, 0x20
ST R0, R1
DATA R0, 0x3DE
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3DF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x208
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x209
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x20A
DATA R1, 0x82
ST R0, R1
DATA R0, 0x20B
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x20C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x20D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x20E
DATA R1, 0x82
ST R0, R1
DATA R0, 0x20F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x188
DATA R1, 0x38
ST R0, R1
DATA R0, 0x189
DATA R1, 0x58
ST R0, R1
DATA R0, 0x18A
DATA R1, 0x18
ST R0, R1
DATA R0, 0x18B
DATA R1, 0x18
ST R0, R1
DATA R0, 0x18C
DATA R1, 0x18
ST R0, R1
DATA R0, 0x18D
DATA R1, 0x18
ST R0, R1
DATA R0, 0x18E
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x18F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x218
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x219
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x21A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x21B
DATA R1, 0x80
ST R0, R1
DATA R0, 0x21C
DATA R1, 0x80
ST R0, R1
DATA R0, 0x21D
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x21E
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x21F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x240
DATA R1, 0x82
ST R0, R1
DATA R0, 0x241
DATA R1, 0x82
ST R0, R1
DATA R0, 0x242
DATA R1, 0x82
ST R0, R1
DATA R0, 0x243
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x244
DATA R1, 0x82
ST R0, R1
DATA R0, 0x245
DATA R1, 0x82
ST R0, R1
DATA R0, 0x246
DATA R1, 0x82
ST R0, R1
DATA R0, 0x247
DATA R1, 0x0
ST R0, R1
DATA R0, 0x258
DATA R1, 0xC4
ST R0, R1
DATA R0, 0x259
DATA R1, 0xC8
ST R0, R1
DATA R0, 0x25A
DATA R1, 0xF0
ST R0, R1
DATA R0, 0x25B
DATA R1, 0xE0
ST R0, R1
DATA R0, 0x25C
DATA R1, 0xD8
ST R0, R1
DATA R0, 0x25D
DATA R1, 0xC4
ST R0, R1
DATA R0, 0x25E
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x25F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x348
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x349
DATA R1, 0x10
ST R0, R1
DATA R0, 0x34A
DATA R1, 0x10
ST R0, R1
DATA R0, 0x34B
DATA R1, 0x10
ST R0, R1
DATA R0, 0x34C
DATA R1, 0x10
ST R0, R1
DATA R0, 0x34D
DATA R1, 0x10
ST R0, R1
DATA R0, 0x34E
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x34F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x108
DATA R1, 0x10
ST R0, R1
DATA R0, 0x109
DATA R1, 0x10
ST R0, R1
DATA R0, 0x10A
DATA R1, 0x10
ST R0, R1
DATA R0, 0x10B
DATA R1, 0x10
ST R0, R1
DATA R0, 0x10C
DATA R1, 0x10
ST R0, R1
DATA R0, 0x10D
DATA R1, 0x0
ST R0, R1
DATA R0, 0x10E
DATA R1, 0x10
ST R0, R1
DATA R0, 0x10F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x210
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x211
DATA R1, 0x86
ST R0, R1
DATA R0, 0x212
DATA R1, 0x82
ST R0, R1
DATA R0, 0x213
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x214
DATA R1, 0x82
ST R0, R1
DATA R0, 0x215
DATA R1, 0x86
ST R0, R1
DATA R0, 0x216
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x217
DATA R1, 0x0
ST R0, R1
DATA R0, 0x2B8
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2B9
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2BA
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2BB
DATA R1, 0x92
ST R0, R1
DATA R0, 0x2BC
DATA R1, 0xBA
ST R0, R1
DATA R0, 0x2BD
DATA R1, 0xAA
ST R0, R1
DATA R0, 0x2BE
DATA R1, 0x44
ST R0, R1
DATA R0, 0x2BF
DATA R1, 0x0
ST R0, R1
DATA R0, 0x328
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x329
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x32A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x32B
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x32C
DATA R1, 0x80
ST R0, R1
DATA R0, 0x32D
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x32E
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x32F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x368
DATA R1, 0x66
ST R0, R1
DATA R0, 0x369
DATA R1, 0xAA
ST R0, R1
DATA R0, 0x36A
DATA R1, 0x92
ST R0, R1
DATA R0, 0x36B
DATA R1, 0x92
ST R0, R1
DATA R0, 0x36C
DATA R1, 0x82
ST R0, R1
DATA R0, 0x36D
DATA R1, 0x82
ST R0, R1
DATA R0, 0x36E
DATA R1, 0x82
ST R0, R1
DATA R0, 0x36F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x390
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x391
DATA R1, 0x82
ST R0, R1
DATA R0, 0x392
DATA R1, 0x82
ST R0, R1
DATA R0, 0x393
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x394
DATA R1, 0xA0
ST R0, R1
DATA R0, 0x395
DATA R1, 0x90
ST R0, R1
DATA R0, 0x396
DATA R1, 0x8E
ST R0, R1
DATA R0, 0x397
DATA R1, 0x0
ST R0, R1
DATA R0, 0x1B0
DATA R1, 0x3E
ST R0, R1
DATA R0, 0x1B1
DATA R1, 0x40
ST R0, R1
DATA R0, 0x1B2
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x1B3
DATA R1, 0x84
ST R0, R1
DATA R0, 0x1B4
DATA R1, 0x82
ST R0, R1
DATA R0, 0x1B5
DATA R1, 0x86
ST R0, R1
DATA R0, 0x1B6
DATA R1, 0xFC
ST R0, R1
DATA R0, 0x1B7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x128
DATA R1, 0xC2
ST R0, R1
DATA R0, 0x129
DATA R1, 0xC4
ST R0, R1
DATA R0, 0x12A
DATA R1, 0x8
ST R0, R1
DATA R0, 0x12B
DATA R1, 0x10
ST R0, R1
DATA R0, 0x12C
DATA R1, 0x20
ST R0, R1
DATA R0, 0x12D
DATA R1, 0x4C
ST R0, R1
DATA R0, 0x12E
DATA R1, 0x8C
ST R0, R1
DATA R0, 0x12F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x288
DATA R1, 0x78
ST R0, R1
DATA R0, 0x289
DATA R1, 0x84
ST R0, R1
DATA R0, 0x28A
DATA R1, 0x84
ST R0, R1
DATA R0, 0x28B
DATA R1, 0x84
ST R0, R1
DATA R0, 0x28C
DATA R1, 0x94
ST R0, R1
DATA R0, 0x28D
DATA R1, 0x8C
ST R0, R1
DATA R0, 0x28E
DATA R1, 0x76
ST R0, R1
DATA R0, 0x28F
DATA R1, 0x7
ST R0, R1
DATA R0, 0x2D0
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x2D1
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2D2
DATA R1, 0xC
ST R0, R1
DATA R0, 0x2D3
DATA R1, 0x38
ST R0, R1
DATA R0, 0x2D4
DATA R1, 0x60
ST R0, R1
DATA R0, 0x2D5
DATA R1, 0x82
ST R0, R1
DATA R0, 0x2D6
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x2D7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x398
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x399
DATA R1, 0x80
ST R0, R1
DATA R0, 0x39A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x39B
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x39C
DATA R1, 0x4
ST R0, R1
DATA R0, 0x39D
DATA R1, 0x4
ST R0, R1
DATA R0, 0x39E
DATA R1, 0xF8
ST R0, R1
DATA R0, 0x39F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x190
DATA R1, 0x7C
ST R0, R1
DATA R0, 0x191
DATA R1, 0x82
ST R0, R1
DATA R0, 0x192
DATA R1, 0x1C
ST R0, R1
DATA R0, 0x193
DATA R1, 0x20
ST R0, R1
DATA R0, 0x194
DATA R1, 0x40
ST R0, R1
DATA R0, 0x195
DATA R1, 0x80
ST R0, R1
DATA R0, 0x196
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x197
DATA R1, 0x0
ST R0, R1
DATA R0, 0x140
DATA R1, 0x8
ST R0, R1
DATA R0, 0x141
DATA R1, 0x10
ST R0, R1
DATA R0, 0x142
DATA R1, 0x20
ST R0, R1
DATA R0, 0x143
DATA R1, 0x20
ST R0, R1
DATA R0, 0x144
DATA R1, 0x20
ST R0, R1
DATA R0, 0x145
DATA R1, 0x10
ST R0, R1
DATA R0, 0x146
DATA R1, 0x8
ST R0, R1
DATA R0, 0x147
DATA R1, 0x0
ST R0, R1
DATA R0, 0x228
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x229
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x22A
DATA R1, 0x80
ST R0, R1
DATA R0, 0x22B
DATA R1, 0xFE
ST R0, R1
DATA R0, 0x22C
DATA R1, 0x80
ST R0, R1
DATA R0, 0x22D
DATA R1, 0xC0
ST R0, R1
DATA R0, 0x22E
DATA R1, 0x7E
ST R0, R1
DATA R0, 0x22F
DATA R1, 0x0
ST R0, R1
DATA R0, 0x388
DATA R1, 0x78
ST R0, R1
DATA R0, 0x389
DATA R1, 0x84
ST R0, R1
DATA R0, 0x38A
DATA R1, 0x84
ST R0, R1
DATA R0, 0x38B
DATA R1, 0x84
ST R0, R1
DATA R0, 0x38C
DATA R1, 0x94
ST R0, R1
DATA R0, 0x38D
DATA R1, 0x8C
ST R0, R1
DATA R0, 0x38E
DATA R1, 0x76
ST R0, R1
DATA R0, 0x38F
DATA R1, 0x7
ST R0, R1
DATA R0, 0x3C0
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x3C1
DATA R1, 0x44
ST R0, R1
DATA R0, 0x3C2
DATA R1, 0x28
ST R0, R1
DATA R0, 0x3C3
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3C4
DATA R1, 0x28
ST R0, R1
DATA R0, 0x3C5
DATA R1, 0x44
ST R0, R1
DATA R0, 0x3C6
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x3C7
DATA R1, 0x0
ST R0, R1
DATA R0, 0x3C8
DATA R1, 0xC6
ST R0, R1
DATA R0, 0x3C9
DATA R1, 0x44
ST R0, R1
DATA R0, 0x3CA
DATA R1, 0x28
ST R0, R1
DATA R0, 0x3CB
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3CC
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3CD
DATA R1, 0x10
ST R0, R1
DATA R0, 0x3CE
DATA R1, 0x38
ST R0, R1
DATA R0, 0x3CF
DATA R1, 0x0
ST R0, R1
JR R3
ROUTINE-io-drawFontCharacter:
DATA R2, %CALL-RETURN-ADDRESS
ST R2, R3
DATA R2, %PEN-POSITION-ADDR
LD R2, R2
DATA R0, 0xFF00
DATA R1, 0x0
ST R0, R1
DATA R3, %KEYCODE-REGISTER
LD R3, R3
DATA R1, 0x101
CMP R3, R1
JMPE ROUTINE-io-drawFontCharacter-carriage-return
DATA R3, %DISPLAY-ADAPTER-ADDR
OUT Addr, R3
ROUTINE-io-drawFontCharacter-STARTLOOP:
DATA R3, %KEYCODE-REGISTER
LD R3, R3
SHL R3
SHL R3
SHL R3
DATA R0, 0xFF00
LD R0, R0
ADD R0, R3
DATA R1, %ONE
ADD R1, R0
DATA R1, 0xFF00
ST R1, R0
LD R3, R0
OUT Data, R2
OUT Data, R0
DATA R1, %LINE-WIDTH
ADD R1, R2
DATA R0, 0xFF00
LD R0, R0
DATA R1, 0x7
CMP R0, R1
JMPE ROUTINE-io-drawFontCharacter-ENDLOOP
JMP ROUTINE-io-drawFontCharacter-STARTLOOP
ROUTINE-io-drawFontCharacter-ENDLOOP:
DATA R1, %LINEX
LD R1, R1
DATA R2, %ONE
ADD R2, R1
DATA R2, %LINEX
ST R2, R1
DATA R0, %PEN-POSITION-ADDR
LD R0, R0
DATA R3, 0x1E
CMP R1, R3
JMPE ROUTINE-io-drawFontCharacter-carriage-return
JMP ROUTINE-io-drawFontCharacter-increment-cursor
ROUTINE-io-drawFontCharacter-increment-cursor:
DATA R1, %ONE
ADD R1, R0
DATA R1, %PEN-POSITION-ADDR
ST R1, R0
JMP ROUTINE-io-drawFontCharacter-deselectIO
ROUTINE-io-drawFontCharacter-carriage-return:
DATA R1, %LINEX
LD R1, R1
DATA R2, 0x0
DATA R3, 0xF0
CMP R1, R2
JMPE ROUTINE-io-drawFontCharacter-reposition-pen
DATA R2, %ONE
DATA R3, 0xEF
CMP R1, R2
JMPE ROUTINE-io-drawFontCharacter-reposition-pen
DATA R2, %LINE-WIDTH
DATA R3, 0xF1
CMP R1, R2
JMPE ROUTINE-io-drawFontCharacter-reposition-pen
ROUTINE-io-drawFontCharacter-reposition-pen-when-midline:
DATA R2, %ONE
DATA R0, 0xEF
NOT R1
ADD R2, R1
CLF
ADD R0, R1
DATA R0, %PEN-POSITION-ADDR
LD R0, R0
ADD R1, R0
DATA R1, %PEN-POSITION-ADDR
ST R1, R0
JMP ROUTINE-io-drawFontCharacter-resetlinex
ROUTINE-io-drawFontCharacter-reposition-pen:
DATA R0, %PEN-POSITION-ADDR
LD R0, R0
ADD R3, R0
DATA R1, %PEN-POSITION-ADDR
ST R1, R0
JMP ROUTINE-io-drawFontCharacter-resetlinex
ROUTINE-io-drawFontCharacter-resetlinex:
DATA R2, %LINEX
DATA R3, 0x0
ST R2, R3
JMP ROUTINE-io-drawFontCharacter-deselectIO
ROUTINE-io-drawFontCharacter-deselectIO:
XOR R3, R3
OUT Addr, R3
CLF
DATA R3, %CALL-RETURN-ADDRESS
LD R3, R3
JR R3
ROUTINE-io-backspace:
DATA R2, %CALL-RETURN-ADDRESS
ST R2, R3
DATA R1, %LINEX
LD R1, R1
DATA R2, 0x0
CMP R1, R2
JMPE ROUTINE-io-backspace-done
XOR R2, R2
NOT R2
ADD R2, R1
DATA R2, %LINEX
ST R2, R1
DATA R2, %PEN-POSITION-ADDR
LD R2, R2
XOR R1, R1
NOT R1
ADD R1, R2
DATA R1, %PEN-POSITION-ADDR
ST R1, R2
DATA R0, 0xFF00
DATA R1, 0x0
ST R0, R1
DATA R3, %DISPLAY-ADAPTER-ADDR
OUT Addr, R3
ROUTINE-io-backspace-clear-STARTLOOP:
DATA R0, 0x0
OUT Data, R2
OUT Data, R0
DATA R1, %LINE-WIDTH
CLF
ADD R1, R2
DATA R0, 0xFF00
LD R0, R0
DATA R1, %ONE
CLF
ADD R1, R0
DATA R1, 0xFF00
ST R1, R0
DATA R1, 0x7
CMP R0, R1
JMPE ROUTINE-io-backspace-clear-ENDLOOP
JMP ROUTINE-io-backspace-clear-STARTLOOP
ROUTINE-io-backspace-clear-ENDLOOP:
XOR R3, R3
OUT Addr, R3
CLF
ROUTINE-io-backspace-done:
DATA R3, %CALL-RETURN-ADDRESS
LD R3, R3
JR R3
ROUTINE-io-pollKeyboard:
DATA R2, %CALL-RETURN-ADDRESS
ST R2, R3
DATA R2, 0xF
OUT Addr, R2
ROUTINE-io-pollKeyboard-STARTLOOP:
IN Data, R3
AND R3, R3
JMPZ ROUTINE-io-pollKeyboard-STARTLOOP
ROUTINE-io-pollKeyboard-ENDLOOP:
DATA R0, %KEYCODE-REGISTER
ST R0, R3
XOR R2, R2
OUT Addr, R2
CLF
DATA R3, %CALL-RETURN-ADDRESS
LD R3, R3
JR R3
main:
DATA R0, %PEN-POSITION-ADDR
DATA R1, 0xF0
ST R0, R1
main-getInput:
CALL ROUTINE-io-pollKeyboard
DATA R0, %KEYCODE-REGISTER
LD R0, R0
DATA R1, 0x103
CMP R0, R1
JMPE main-handle-backspace
CALL ROUTINE-io-drawFontCharacter
JMP main-getInput
main-handle-backspace:
CALL ROUTINE-io-backspace
JMP main-getInput
